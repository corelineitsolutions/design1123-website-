...
import { referralSections, LEGAL } from "@/lib/legal";
import { ArrowUp, Gift, Calendar, Mail, FileText, ChevronRight } from "lucide-react";
...
export default function ReferralPolicy() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      for (const section of referralSections) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between w-full overflow-x-hidden bg-bg-dark text-white">
      ...
      <div className="w-full relative pt-32 pb-20 overflow-hidden border-b border-border-glass bg-bg-section/50">
        ...
        {/* Glow meshes */}
        <div className="absolute -left-20 top-0 w-[500px] h-[500px] bg-brand-purple/10 rounded-full filter blur-[120px] pointer-events-none" />
        <div className="absolute -right-20 bottom-0 w-[500px] h-[500px] bg-brand-orange/10 rounded-full filter blur-[120px] pointer-events-none" />
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-text-secondary">Community & Rewards</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-heading font-black tracking-tight text-white mb-6 leading-tight">
              Referral <span className="text-gradient">Policy</span>
            </h1>
            <p className="text-text-secondary text-lg md:text-xl leading-relaxed mb-8 max-w-2xl">
              Invite other businesses to Design 1123, help them grow their branding, and earn premium subscription extensions and credits.
            </p>
            <div className="flex flex-wrap gap-6 text-sm text-text-secondary border-t border-white/10 pt-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Program: <strong className="text-white">Active (Refer & Earn)</strong></span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="container mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
                  ...
                  {referralSections.map((section) => (
                    <button
                      ...
                      className={`flex items-center justify-between text-left px-3 py-2.5 rounded-xl transition-all duration-300 text-xs font-bold uppercase tracking-wider ${
                        activeSection === section.id
                          ? "bg-white/10 text-white border-l-2 border-brand-purple pl-4"
                    ...
                    >
                      ...
                      <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-300 ${
                        activeSection === section.id ? "translate-x-0.5 text-brand-purple" : "opacity-0"
                      }`} />
                    </button>
                  ))}
                </nav>
                <div className="pt-4 border-t border-white/10">
                  <div className="p-4 rounded-2xl bg-gradient-to-tr from-brand-purple/10 to-brand-orange/5 border border-brand-purple/20 text-center">
                    <p className="text-xs font-bold uppercase tracking-wider text-white mb-2">Need Assistance?</p>
                    <p className="text-[11px] text-text-secondary mb-3 leading-relaxed">
                      Have questions about your referral code, tracking, or reward status?
                    </p>
                    <a 
                      href={`mailto:${LEGAL.contactEmail}`}
                      className="inline-flex items-center gap-1.5 text-xs text-brand-purple font-bold uppercase hover:underline"
                    >
            ...
            <div className="lg:col-span-8 space-y-12">
              {referralSections.map((section, idx) => (
                <motion.section
                  key={section.id}
                  id={section.id}
                ...
                >
                  {activeSection === section.id && (
                    <div className="absolute inset-0 border border-brand-purple/30 rounded-[24px] pointer-events-none" />
                  )}

                  <h2 className="text-2xl font-heading font-black text-white mb-6 group-hover:text-brand-purple transition-colors duration-300">
                    {section.title}
                  </h2>
                    ...
                    <ul className="mt-6 space-y-3.5">
                      {section.list.map((item, lIdx) => (
                        <li key={lIdx} className="flex items-start gap-3 text-text-secondary text-sm md:text-base">
                          <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-purple shrink-0 shadow-[0_0_8px_#9C27B0]" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="fixed bottom-6 right-6 z-40 w-12 h-12 rounded-full bg-brand-purple text-white flex items-center justify-center shadow-lg shadow-brand-purple/30 hover:bg-brand-deep-purple transition-all hover:scale-110 cursor-pointer"
