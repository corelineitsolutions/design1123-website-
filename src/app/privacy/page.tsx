...
import { privacySections, LEGAL } from "@/lib/legal";
import { ArrowUp, Shield, Calendar, Mail, FileText, ChevronRight } from "lucide-react";
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-text-secondary">Legal Compliance</span>
            </div>
            ...
            <p className="text-text-secondary text-lg md:text-xl leading-relaxed mb-8 max-w-2xl">
              We value your trust. This Privacy Policy details how Design 1123 collects, uses, and safeguards your business and personal information.
            </p>
            <div className="flex flex-wrap gap-6 text-sm text-text-secondary border-t border-white/10 pt-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Applicable to: <strong className="text-white">All Users & Creators</strong></span>
              </div>
            </div>
          </motion.div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
                  ...
                  {privacySections.map((section) => (
                    <button
                      ...
                      className={`flex items-center justify-between text-left px-3 py-2.5 rounded-xl transition-all duration-300 text-xs font-bold uppercase tracking-wider ${
                        activeSection === section.id
                          ? "bg-white/10 text-white border-l-2 border-brand-orange pl-4"
                    ...
                    >
                      ...
                      <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-300 ${
                        activeSection === section.id ? "translate-x-0.5 text-brand-orange" : "opacity-0"
                      }`} />
                    </button>
                  ))}
                </nav>
                <div className="pt-4 border-t border-white/10">
                  <div className="p-4 rounded-2xl bg-gradient-to-tr from-brand-orange/10 to-brand-pink/5 border border-brand-orange/20 text-center">
                    ...
                    <p className="text-[11px] text-text-secondary mb-3 leading-relaxed">
                      Have questions about your data rights or compliance policies?
                    </p>
                    <a 
                      href={`mailto:${LEGAL.contactEmail}`}
                      className="inline-flex items-center gap-1.5 text-xs text-brand-orange font-bold uppercase hover:underline"
                    >
            ...
            <div className="lg:col-span-8 space-y-12">
              {privacySections.map((section, idx) => (
                ...
                >
                  {activeSection === section.id && (
                    <div className="absolute inset-0 border border-brand-orange/30 rounded-[24px] pointer-events-none" />
                  )}

                  <h2 className="text-2xl font-heading font-black text-white mb-6 group-hover:text-brand-orange transition-colors duration-300">
                    {section.title}
                  </h2>
                    ...
                    <ul className="mt-6 space-y-3.5">
                      {section.list.map((item, lIdx) => (
                        <li key={lIdx} className="flex items-start gap-3 text-text-secondary text-sm md:text-base">
                          <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-orange shrink-0 shadow-[0_0_8px_#FF8A00]" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="fixed bottom-6 right-6 z-40 w-12 h-12 rounded-full bg-brand-orange text-white flex items-center justify-center shadow-lg shadow-brand-orange/30 hover:bg-brand-deep-orange transition-all hover:scale-110 cursor-pointer"
